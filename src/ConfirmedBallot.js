...
function Result({order, code, name, title, ranks, points}){
    ...
    <span className="half centeralign marno padno">Rank: {ranks} – {points}</span>
  </div>
}
...
export default function ConfirmedBallot({ entries, editFeedback }) {
  return (
      ...
      <span className="twothirds rightalign semibold nospace">
        <p>
          Ballots will appear here once rounds have been published on the web
        </p>
      </span>
      ...
      <p className="martop marbottom bluetext semibold">
        ...
        have confirmed a ballot, you must contact the tournament officials to
        make changes. You may edit/extend past comments until the tournament
        ends.
      </p>
...
{entries.map((e,i)=><Result {...e} className={`row ${i % 2 === 0 ? "even" : "odd"}`}/>)}
    </>}
    />
  );
}
